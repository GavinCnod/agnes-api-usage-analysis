...
/** 保留旧 localStorage key，避免已有浏览器本地署名丢失。 */
...
export default function ShareModal({ tab, onClose }: ShareModalProps) {
  ...
  const cardStrings: ShareCardStrings = useMemo(() => ({
    ...
    textTokensLabel: t.metrics.tabTextTokens ?? t.metrics.textTokens,
    imagesLabel: t.metrics.tabImages ?? t.metrics.images,
    videoSecondsLabel: t.metrics.tabVideoSeconds ?? t.metrics.videoSeconds,
    ...
    relativeComparisonLabel: t.share.relativeComparison ?? "Relative Comparison",
    dailyCoreMetricsLabel: t.overview.dailyCoreMetrics ?? t.overview.chartMetricLabel,
    coreMetricsTrendLabel: t.trends?.coreMetricsTrend ?? t.trends?.activeMetric ?? t.overview.chartMetricLabel,
...
function downloadBlob(blob: Blob): void {
  ...
  a.href = url;
  a.download = "agnes-usage-share.png";
